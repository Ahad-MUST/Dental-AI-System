...
import { Activity, Calendar, Clock, Clock12, BarChart3 } from 'lucide-react';
import Card from '../common/Card';
import { CHART_CONFIG, CHART_COLORS } from '../../utils/constants';

const CallTypesChart = ({ data, rawData }) => {
  ...
  const getFilteredData = useMemo(() => {
    ...
    // Process filtered data to get call types distribution
    const callTypes = filteredData.reduce((acc, item) => {
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-white/95 backdrop-blur-sm border border-slate-200 rounded-lg shadow-lg p-3">
          <p className="text-sm font-semibold text-slate-900 mb-1">{payload[0].payload.type}</p>
        ...
        {payload.map((entry, index) => (
          <div key={index} className="flex items-center justify-between p-2 rounded-lg bg-slate-50 hover:bg-slate-100 transition-colors">
            ...
            <span className="text-sm font-semibold text-slate-900">
              {getFilteredData.find(item => item.type === entry.value)?.percentage}%
            </span>
          </div>
        ))}
      </div>
    ...
    <Card className="h-full">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h3 className="text-lg font-semibold text-slate-900">Call Distribution</h3>
            <p className="text-sm text-slate-600">
              {selectedPeriodData?.description} by call type
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="h-64 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-slate-500 font-medium">
              No call type data available for {selectedPeriodData?.description.toLowerCase()}
            </p>
